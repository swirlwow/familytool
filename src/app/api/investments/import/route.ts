...
const aliases: Record<string, string> = { 日期: "date", date: "date", 類型: "type", type: "type", 券商帳戶: "account", account: "account", 市場: "market", market: "market", 股票代號: "symbol", symbol: "symbol", 股票名稱: "name", name: "name", 股數: "quantity", quantity: "quantity", 成交價: "price", price: "price", 手續費: "fee", fee: "fee", 交易稅: "tax", tax: "tax", 股利金額: "cash", cash_amount: "cash", 備註: "note", note: "note" };
...
export async function POST(request: Request) {
  try {
    ...
    for (let index = 1; index < rows.length; index += 1) {
      ...
      await createInvestmentRecord(workspaceId, "transaction", { account_id: account.id, security_id: security.id, transaction_type: transactionType, trade_date: values.date, quantity: values.quantity, price: values.price, fee: values.fee, tax: values.tax, cash_amount: values.cash, note: values.note });

...
import "./investment-colors.css";
import controls from "@/components/ui/record-controls.module.css";

import { useCallback, useEffect, useMemo, useState } from "react";
import { ArrowDownToLine, Archive, ChartNoAxesColumnIncreasing, ChartPie, Gift, Layers, Minus, UserRound, Building2, Coins, Landmark, Plus, RefreshCw, Search, TrendingDown, TrendingUp } from "lucide-react";
...
type CsvExportScope = Extract<Tab, "transactions" | "dividends" | "corporate_actions">;
...
const PREFERRED_ACCOUNT_NAME = "永豐-大美女";
...
export default function InvestmentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = json.data as InvestmentSnapshot;
      setSnapshot(data);
      setAccountFilter((current) => data.accounts.some((account) => account.id === current)
        ? current
        : data.accounts.find((account) => account.name === PREFERRED_ACCOUNT_NAME)?.id ?? data.accounts.find((account) => account.name.includes("大美女"))?.id ?? data.accounts[0]?.id ?? "all");
    } catch (error) {
  ...
  const accountTradedSecurities = useMemo(() => {
    if (accountFilter === "all") return snapshot.securities;
    const tradedSecurityIds = new Set(snapshot.transactions
      .filter((row) => row.account_id === accountFilter && row.transaction_type !== "dividend")
      .map((row) => row.security_id));
    return snapshot.securities.filter((row) => tradedSecurityIds.has(row.id));
  }, [accountFilter, snapshot.securities, snapshot.transactions]);
  ...
  const filteredTransactions = useMemo(() => snapshot.transactions.filter((row) => row.transaction_type !== "dividend" && (typeFilter === "all" || row.transaction_type === typeFilter) && (!dateFrom || row.trade_date >= dateFrom) && (!dateTo || row.trade_date <= dateTo) && matches(row.account_id, row.security_id, row.note)), [snapshot.transactions, typeFilter, dateFrom, dateTo, matches]);
  ...
  function download(format: "csv" | "json", scope?: CsvExportScope) {
    if (!WORKSPACE_ID) return; const link = document.createElement("a");
    const params = new URLSearchParams({ workspace_id: WORKSPACE_ID, format });
    if (format === "csv" && scope) {
      params.set("scope", scope);
      if (accountFilter !== "all") params.set("account_id", accountFilter);
      if (securityFilter !== "all") params.set("security_id", securityFilter);
      if (scope === "transactions" && typeFilter !== "all") params.set("transaction_type", typeFilter);
      if (scope === "transactions" && dateFrom) params.set("date_from", dateFrom);
      if (scope === "transactions" && dateTo) params.set("date_to", dateTo);
      if (query.trim()) params.set("query", query.trim());
    }
    link.href = `/api/investments/export?${params.toString()}`;
  ...
  const selectedAccount = accountMap.get(accountFilter) ?? snapshot.accounts.find((account) => account.name === PREFERRED_ACCOUNT_NAME) ?? snapshot.accounts[0];
  const accountSummary = selectedAccount ? (() => {
    const rows = snapshot.holdings.filter((row) => row.account_id === selectedAccount.id);
    const openRows = rows.filter((row) => row.quantity > 0);
    const costBasis = openRows.reduce((sum, row) => sum + row.cost_basis, 0);
    const marketValue = openRows.every((row) => row.market_value !== null) ? openRows.reduce((sum, row) => sum + Number(row.market_value), 0) : null;
    const estimatedSaleValue = openRows.every((row) => row.estimated_sale_value !== null) ? openRows.reduce((sum, row) => sum + Number(row.estimated_sale_value), 0) : null;
    const dividendGross = openRows.reduce((sum, row) => sum + row.position_dividend_gross, 0);
    const adjustedCostBasis = costBasis - dividendGross;
    const unrealizedProfit = estimatedSaleValue === null ? null : estimatedSaleValue - costBasis;
    const dividendAdjustedProfit = estimatedSaleValue === null ? null : estimatedSaleValue - adjustedCostBasis;
    return { account: selectedAccount, stats: [
      { label: "持有成本", value: wholeMoney(costBasis), detail: `除息後 ${wholeMoney(adjustedCostBasis)}`, icon: Coins, color: "text-sky-600", bg: "bg-sky-50" },
      { label: "預估賣出淨值", value: wholeMoney(estimatedSaleValue), detail: `市值 ${wholeMoney(marketValue)}`, icon: ChartPie, color: "text-indigo-600", bg: "bg-indigo-50" },
      { label: "未實現損益", value: signedWholeMoney(unrealizedProfit), detail: "不含股利", icon: (unrealizedProfit ?? 0) >= 0 ? TrendingUp : TrendingDown, color: (unrealizedProfit ?? 0) >= 0 ? "text-rose-600" : "text-emerald-600", bg: (unrealizedProfit ?? 0) >= 0 ? "bg-rose-50" : "bg-emerald-50" },
      { label: "含股利總損益", value: signedWholeMoney(dividendAdjustedProfit), detail: `累計除息 ${wholeMoney(dividendGross)}`, icon: Gift, color: (dividendAdjustedProfit ?? 0) >= 0 ? "text-rose-600" : "text-emerald-600", bg: (dividendAdjustedProfit ?? 0) >= 0 ? "bg-rose-50" : "bg-emerald-50" },
    ] };
  })() : null;
  const tabs: [Tab, string][] = [["holdings", "現有持股"], ["transactions", "交易紀錄"], ["dividends", "股利紀錄"], ["corporate_actions", "股權異動"], ["settings", "基本資料"]];
  const csvExport = tab === "transactions"
    ? { scope: "transactions" as const, label: "匯出交易紀錄" }
    : tab === "dividends"
      ? { scope: "dividends" as const, label: "匯出股利紀錄" }
      : tab === "corporate_actions"
        ? { scope: "corporate_actions" as const, label: "匯出股權異動" }
        : null;

  return <main className="app-page reports-ui investments-ui"><div className="app-page-inner max-w-[1500px]">
    <header className="app-header"><div className="flex min-w-0 items-center gap-3"><span className="rounded-lg bg-indigo-50 p-2 text-indigo-600"><TrendingUp className="h-5 w-5" /></span><div className="min-w-0"><h1 className="truncate text-lg font-black text-slate-900 sm:text-xl">股票買賣管理</h1><p className="text-xs font-medium text-slate-500">獨立投資紀錄，不連動家庭帳務</p></div></div><div className="flex flex-wrap items-center justify-end gap-2"><button className={controls.button} onClick={() => void load()} aria-label="重新讀取"><RefreshCw className="h-4 w-4" /></button><button className={controls.button} onClick={() => setModal({ kind: "account" })}><Landmark className="h-4 w-4" />帳戶</button><button className={controls.button} onClick={() => setModal({ kind: "security" })}><Building2 className="h-4 w-4" />股票</button><button className={controls.button} onClick={() => setModal({ kind: "transaction", transactionType: "buy", accountId: selectedAccount?.id })}><Plus className="h-4 w-4" />新增交易</button></div></header>
    <section>{accountSummary ? <article className="app-panel overflow-hidden"><header className="flex flex-col gap-3 border-b border-slate-100 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><span className="rounded-lg bg-indigo-50 p-2 text-indigo-600"><UserRound className="h-4 w-4" /></span><div className="min-w-0"><h2 className="truncate text-sm font-black text-slate-800">{accountSummary.account.name}</h2><p className="truncate text-xs text-slate-400">{accountSummary.account.broker || "未填券商"}</p></div></div><div role="tablist" aria-label="切換券商帳戶" className="-mx-1 flex max-w-full gap-1 overflow-x-auto p-1 sm:mx-0">{snapshot.accounts.map((account) => <button key={account.id} type="button" role="tab" aria-selected={account.id === accountSummary.account.id} onClick={() => { setAccountFilter(account.id); setSecurityFilter("all"); }} className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${account.id === accountSummary.account.id ? "border-indigo-200 bg-indigo-600 text-white shadow-sm" : "border-slate-200 bg-white text-slate-600 hover:border-indigo-200 hover:text-indigo-700"}`}>{account.name}</button>)}</div></header><div className="grid grid-cols-2 gap-px bg-slate-100 xl:grid-cols-4">{accountSummary.stats.map(({ label, value, detail, icon: Icon, color, bg }) => <div key={label} className="flex min-w-0 items-center gap-3 bg-white p-4"><span className={`rounded-lg p-2.5 ${bg} ${color}`}><Icon className="h-5 w-5" /></span><div className="min-w-0"><p className="text-xs font-bold text-slate-500">{label}</p><p className={`truncate text-lg font-black sm:text-xl ${color}`}>{loading ? "—" : value}</p><p className="truncate text-[11px] text-slate-400">{loading ? "—" : detail}</p></div></div>)}</div></article> : <div className="app-panel app-empty">{loading ? "正在讀取帳戶…" : "尚未建立券商帳戶"}</div>}</section>
    <section className="app-panel overflow-hidden">
      <div className={`${controls.investmentToolbar} border-b border-slate-200 p-3`}><div className={`${controls.investmentTabs} rounded-lg bg-slate-100 p-1`}>{tabs.map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={`shrink-0 rounded-md px-4 py-2 text-sm font-bold transition ${tab === key ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500"}`}>{label}</button>)}</div><div className={controls.investmentActions}><button type="button" className={controls.button} onClick={() => { setTab("transactions"); }}><ChartNoAxesColumnIncreasing className="h-4 w-4" />單股收益</button>{tab === "holdings" && <button className={controls.button} onClick={() => void updateHoldingPrices()} disabled={updatingPrices || snapshot.holdings.length === 0}><RefreshCw className={`h-4 w-4 ${updatingPrices ? "animate-spin" : ""}`} />{updatingPrices ? "更新中" : "更新持股股價"}</button>}<button className={controls.button} onClick={() => setModal({ kind: "transaction", transactionType: "buy", accountId: selectedAccount?.id })}><Plus className="h-4 w-4" />新增買進</button><button className={controls.button} onClick={() => setModal({ kind: "transaction", transactionType: "sell", accountId: selectedAccount?.id })}><Minus className="h-4 w-4" />新增賣出</button><button className={controls.button} onClick={() => setModal({ kind: "dividend", accountId: selectedAccount?.id })}><Gift className="h-4 w-4" />新增股利</button><button className={controls.button} onClick={() => setModal({ kind: "corporate_action", accountId: selectedAccount?.id })}><Layers className="h-4 w-4" />新增減資</button>{csvExport && <button className={controls.button} onClick={() => download("csv", csvExport.scope)}><ArrowDownToLine className="h-4 w-4" />{csvExport.label}</button>}<button className={controls.button} onClick={() => download("json")}><Archive className="h-4 w-4" />備份</button></div></div>
      {tab !== "settings" && <div className={`grid gap-2 border-b border-slate-100 bg-slate-50/70 p-3 sm:grid-cols-2 ${tab === "transactions" ? "xl:grid-cols-6" : "xl:grid-cols-3"}`}>
        <label className="input input-sm flex items-center gap-2 rounded-lg border-slate-200 bg-white"><Search className="h-4 w-4 text-slate-400" /><input className="grow" placeholder="搜尋股票或代號" value={query} onChange={(event) => setQuery(event.target.value)} /></label>
        <select aria-label="選擇股票查詢單股收益" className="select select-sm rounded-lg border-slate-200 bg-white" value={securityFilter} onChange={(event) => setSecurityFilter(event.target.value)}><option value="all">全部股票</option>{accountTradedSecurities.map((row) => <option key={row.id} value={row.id}>{row.symbol} {row.name}</option>)}</select>
        {tab === "transactions" && <>
          <select className="select select-sm rounded-lg border-slate-200 bg-white" value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as InvestmentTransactionType | "all")}><option value="all">全部類型</option><option value="buy">買進</option><option value="sell">賣出</option></select>
          <label className="input input-sm flex items-center gap-2 rounded-lg border-slate-200 bg-white"><span className="shrink-0 text-xs font-bold text-slate-500">開始</span><input type="date" aria-label="交易開始日期" className="min-w-0 grow bg-transparent text-xs outline-none" value={dateFrom} max={dateTo || undefined} onChange={(event) => setDateFrom(event.target.value)} /></label>
          <label className="input input-sm flex items-center gap-2 rounded-lg border-slate-200 bg-white"><span className="shrink-0 text-xs font-bold text-slate-500">結束</span><input type="date" aria-label="交易結束日期" className="min-w-0 grow bg-transparent text-xs outline-none" value={dateTo} min={dateFrom || undefined} onChange={(event) => setDateTo(event.target.value)} /></label>
          <button type="button" className={controls.button} disabled={!dateFrom && !dateTo} onClick={() => { setDateFrom(""); setDateTo(""); }}>清除日期</button>
        </>}
      </div>}
      {tab === "transactions" && !selectedPerformance && <p className="border-b border-slate-100 px-4 py-3 text-sm text-slate-500">請從「全部股票」選單選擇一支股票，即可查看單股收益（包含已賣出股票）。</p>}

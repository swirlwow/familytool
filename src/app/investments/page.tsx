...
export default function InvestmentsPage() {
  ...
  return <main className="app-page"><div className="app-page-inner max-w-[1500px]">
    <header className="app-header"><div className="flex min-w-0 items-center gap-3"><span className="rounded-lg bg-indigo-50 p-2 text-indigo-600"><TrendingUp className="h-5 w-5" /></span><div className="min-w-0"><h1 className="truncate text-lg font-black text-slate-900 sm:text-xl">股票買賣管理</h1><p className="text-xs font-medium text-slate-500">獨立投資紀錄，不連動家庭帳務</p></div></div><div className="flex flex-wrap items-center justify-end gap-2"><button className="btn btn-sm rounded-lg border-slate-200 bg-white" onClick={() => void load()} aria-label="重新讀取"><RefreshCw className="h-4 w-4" /></button><button className="btn btn-sm rounded-lg border-slate-200 bg-white text-indigo-700" onClick={() => setModal({ kind: "account" })}><Landmark className="h-4 w-4" />帳戶</button><button className="btn btn-sm rounded-lg border-slate-200 bg-white text-indigo-700" onClick={() => setModal({ kind: "security" })}><Building2 className="h-4 w-4" />股票</button><button className="btn btn-sm rounded-lg border-0 bg-indigo-600 text-white" onClick={() => setModal({ kind: "transaction", transactionType: "buy", accountId: selectedAccount?.id })}><Plus className="h-4 w-4" />新增交易</button></div></header>
    ...
    <section className="app-panel overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-slate-200 p-3 xl:flex-row xl:items-center xl:justify-between"><div className="flex w-full overflow-x-auto rounded-lg bg-slate-100 p-1 xl:w-auto">{tabs.map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={`shrink-0 rounded-md px-4 py-2 text-sm font-bold transition ${tab === key ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500"}`}>{label}</button>)}</div><div className="flex flex-wrap gap-2">{tab === "holdings" && <button className="btn btn-sm rounded-lg border-sky-200 bg-sky-50 text-sky-700" onClick={() => void updateHoldingPrices()} disabled={updatingPrices || snapshot.holdings.length === 0}><RefreshCw className={`h-4 w-4 ${updatingPrices ? "animate-spin" : ""}`} />{updatingPrices ? "更新中" : "更新持股股價"}</button>}<button className="btn btn-sm rounded-lg border-0 bg-sky-600 text-white" onClick={() => setModal({ kind: "transaction", transactionType: "buy", accountId: selectedAccount?.id })}><TrendingUp className="h-4 w-4" />新增買進</button><button className="btn btn-sm rounded-lg border-amber-200 bg-amber-50 text-amber-700" onClick={() => setModal({ kind: "transaction", transactionType: "sell", accountId: selectedAccount?.id })}><TrendingDown className="h-4 w-4" />新增賣出</button><button className="btn btn-sm rounded-lg border-emerald-200 bg-emerald-50 text-emerald-700" onClick={() => setModal({ kind: "dividend", accountId: selectedAccount?.id })}><Coins className="h-4 w-4" />新增股利</button><button className="btn btn-sm rounded-lg border-violet-200 bg-violet-50 text-violet-700" onClick={() => setModal({ kind: "corporate_action", accountId: selectedAccount?.id })}>新增減資</button>{csvExport && <button className="btn btn-sm rounded-lg border-slate-200 bg-white" onClick={() => download("csv", csvExport.scope)}><ArrowDownToLine className="h-4 w-4" />{csvExport.label}</button>}<button className="btn btn-sm rounded-lg border-slate-200 bg-white" onClick={() => download("json")}><Download className="h-4 w-4" />備份</button></div></div>
      {tab !== "settings" && <div className={`grid gap-2 border-b border-slate-100 bg-slate-50/70 p-3 sm:grid-cols-2 ${tab === "transactions" ? "xl:grid-cols-6" : "xl:grid-cols-3"}`}>
        ...
        <select className="select select-sm rounded-lg border-slate-200 bg-white" value={securityFilter} onChange={(event) => setSecurityFilter(event.target.value)}><option value="all">全部股票</option>{accountTradedSecurities.map((row) => <option key={row.id} value={row.id}>{row.symbol} {row.name}</option>)}</select>
        {tab === "transactions" && <>
          ...
          <button type="button" className="btn btn-sm rounded-lg border-slate-200 bg-white text-slate-600 disabled:text-slate-300" disabled={!dateFrom && !dateTo} onClick={() => { setDateFrom(""); setDateTo(""); }}>清除日期</button>
        </>}
      </div>}

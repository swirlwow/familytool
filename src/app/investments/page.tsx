...
export default function InvestmentsPage() {
  ...
  return <main className="app-page"><div className="app-page-inner max-w-[1500px]">
    <header className="app-header">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2"><button className="btn btn-sm rounded-lg border-slate-200 bg-white" onClick={() => void load()} aria-label="重新讀取"><RefreshCw className="h-4 w-4" /></button><button className="btn btn-sm rounded-lg border-slate-200 bg-white text-indigo-700" onClick={() => setModal({ kind: "account" })}><Landmark className="h-4 w-4" /><span className="hidden sm:inline">新增帳戶</span><span className="sm:hidden">帳戶</span></button><button className="btn btn-sm rounded-lg border-slate-200 bg-white text-indigo-700" onClick={() => setModal({ kind: "security" })}><Building2 className="h-4 w-4" /><span className="hidden sm:inline">新增股票</span><span className="sm:hidden">股票</span></button><button className="btn btn-sm rounded-lg border-0 bg-indigo-600 text-white hover:bg-indigo-700" onClick={() => openTransaction("buy")}><Plus className="h-4 w-4" /><span className="hidden sm:inline">新增交易</span><span className="sm:hidden">交易</span></button></div>
    </header>
    ...
    <section className="app-panel overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-slate-200 p-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button className="btn btn-sm rounded-lg border-0 bg-sky-600 text-white hover:bg-sky-700" onClick={() => openTransaction("buy")}><TrendingUp className="h-4 w-4" />新增買進</button>
          <button className="btn btn-sm rounded-lg border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100" onClick={() => openTransaction("sell")}><TrendingDown className="h-4 w-4" />新增賣出</button>
          <button className="btn btn-sm rounded-lg border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100" onClick={() => openTransaction("dividend")}><Coins className="h-4 w-4" />股利</button>
          <span className="mx-1 hidden w-px self-stretch bg-slate-200 xl:block" aria-hidden="true" />
...
function RecordModal({ modal, accounts, securities, saving, onClose, onSave }: { modal: Exclude<Modal, null>; accounts: InvestmentAccount[]; securities: InvestmentSecurity[]; saving: boolean; onClose: () => void; onSave: (resource: string, body: Record<string, unknown>, id?: string) => Promise<void> }) {
  ...
  const requestedType = modal.kind === "transaction" ? modal.transactionType : undefined;
  const initialForm = (isTx ? { transaction_type: tx?.transaction_type ?? requestedType ?? "buy", trade_date: tx?.trade_date ?? today(), account_id: tx?.account_id ?? accounts.find((r) => r.is_active)?.id ?? "", security_id: tx?.security_id ?? securities.find((r) => r.is_active)?.id ?? "", quantity: tx?.quantity ? String(tx.quantity) : "", price: tx?.price ? String(tx.price) : "", fee: tx?.fee ? String(tx.fee) : "", tax: tx?.tax ? String(tx.tax) : "", cash_amount: tx?.cash_amount ? String(tx.cash_amount) : "", note: tx?.note ?? "" } : modal.kind === "account" ? { name: account?.name ?? "", broker: account?.broker ?? "", currency: account?.currency ?? "TWD", is_active: account?.is_active ?? true, note: account?.note ?? "" } : { symbol: security?.symbol ?? "", name: security?.name ?? "", market: security?.market ?? "TWSE", currency: security?.currency ?? "TWD", current_price: security?.current_price == null ? "" : String(security.current_price), current_price_date: security?.current_price_date ?? "", is_active: security?.is_active ?? true, note: security?.note ?? "" }) as Record<string, string | boolean>;

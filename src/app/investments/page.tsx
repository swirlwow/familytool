...
import { ArrowDownToLine, BriefcaseBusiness, Building2, Coins, Download, Landmark, Plus, RefreshCw, Search, TrendingDown, TrendingUp, WalletCards } from "lucide-react";
...
export default function InvestmentsPage() {
  ...
  const accountSummary = selectedAccount ? (() => {
    ...
    return { account: selectedAccount, stats: [
      { label: "持有成本", value: wholeMoney(costBasis), detail: `除息後 ${wholeMoney(adjustedCostBasis)}`, icon: WalletCards, color: "text-sky-600", bg: "bg-sky-50" },
      { label: "預估賣出淨值", value: wholeMoney(estimatedSaleValue), detail: `市值 ${wholeMoney(marketValue)}`, icon: BriefcaseBusiness, color: "text-indigo-600", bg: "bg-indigo-50" },
      ...
      { label: "含股利總損益", value: signedWholeMoney(dividendAdjustedProfit), detail: `累計除息 ${wholeMoney(dividendGross)}`, icon: Coins, color: (dividendAdjustedProfit ?? 0) >= 0 ? "text-rose-600" : "text-emerald-600", bg: (dividendAdjustedProfit ?? 0) >= 0 ? "bg-rose-50" : "bg-emerald-50" },
    ] };
  })() : null;
  ...
  return <main className="app-page reports-ui investments-ui"><div className="app-page-inner max-w-[1500px]">
    ...
    <section>{accountSummary ? <article className="app-panel overflow-hidden"><header className="flex flex-col gap-3 border-b border-slate-100 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><span className="rounded-lg bg-indigo-50 p-2 text-indigo-600"><Landmark className="h-4 w-4" /></span><div className="min-w-0"><h2 className="truncate text-sm font-black text-slate-800">{accountSummary.account.name}</h2><p className="truncate text-xs text-slate-400">{accountSummary.account.broker || "未填券商"}</p></div></div><div role="tablist" aria-label="切換券商帳戶" className="-mx-1 flex max-w-full gap-1 overflow-x-auto p-1 sm:mx-0">{snapshot.accounts.map((account) => <button key={account.id} type="button" role="tab" aria-selected={account.id === accountSummary.account.id} onClick={() => { setAccountFilter(account.id); setSecurityFilter("all"); }} className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ${account.id === accountSummary.account.id ? "border-indigo-200 bg-indigo-600 text-white shadow-sm" : "border-slate-200 bg-white text-slate-600 hover:border-indigo-200 hover:text-indigo-700"}`}>{account.name}</button>)}</div></header><div className="grid grid-cols-2 gap-px bg-slate-100 xl:grid-cols-4">{accountSummary.stats.map(({ label, value, detail, icon: Icon, color, bg }) => <div key={label} className="flex min-w-0 items-center gap-3 bg-white p-4"><span className={`rounded-lg p-2.5 ${bg} ${color}`}><Icon className="h-5 w-5" /></span><div className="min-w-0"><p className="text-xs font-bold text-slate-500">{label}</p><p className={`truncate text-lg font-black sm:text-xl ${color}`}>{loading ? "—" : value}</p><p className="truncate text-[11px] text-slate-400">{loading ? "—" : detail}</p></div></div>)}</div></article> : <div className="app-panel app-empty">{loading ? "正在讀取帳戶…" : "尚未建立券商帳戶"}</div>}</section>
    <section className="app-panel overflow-hidden">
      <div className={`${controls.investmentToolbar} border-b border-slate-200 p-3`}><div className={`${controls.investmentTabs} rounded-lg bg-slate-100 p-1`}>{tabs.map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={`shrink-0 rounded-md px-4 py-2 text-sm font-bold transition ${tab === key ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500"}`}>{label}</button>)}</div><div className={controls.investmentActions}><button type="button" className={controls.button} onClick={() => { setTab("transactions"); }}><TrendingUp className="h-4 w-4" />單股收益</button>{tab === "holdings" && <button className={controls.button} onClick={() => void updateHoldingPrices()} disabled={updatingPrices || snapshot.holdings.length === 0}><RefreshCw className={`h-4 w-4 ${updatingPrices ? "animate-spin" : ""}`} />{updatingPrices ? "更新中" : "更新持股股價"}</button>}<button className={controls.button} onClick={() => setModal({ kind: "transaction", transactionType: "buy", accountId: selectedAccount?.id })}><TrendingUp className="h-4 w-4" />新增買進</button><button className={controls.button} onClick={() => setModal({ kind: "transaction", transactionType: "sell", accountId: selectedAccount?.id })}><TrendingDown className="h-4 w-4" />新增賣出</button><button className={controls.button} onClick={() => setModal({ kind: "dividend", accountId: selectedAccount?.id })}><Coins className="h-4 w-4" />新增股利</button><button className={controls.button} onClick={() => setModal({ kind: "corporate_action", accountId: selectedAccount?.id })}>新增減資</button>{csvExport && <button className={controls.button} onClick={() => download("csv", csvExport.scope)}><ArrowDownToLine className="h-4 w-4" />{csvExport.label}</button>}<button className={controls.button} onClick={() => download("json")}><Download className="h-4 w-4" />備份</button></div></div>

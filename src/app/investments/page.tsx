...
export default function InvestmentsPage() {
  ...
  const matches = useCallback((accountId: string, securityId: string, note?: string | null) => {
    ...
    return (accountFilter === "all" || accountId === accountFilter) && (!keyword || [account?.name, security?.symbol, security?.name, security?.market, note].some((value) => String(value ?? "").toLowerCase().includes(keyword)));
  }, [query, accountFilter, accountMap, securityMap]);
  ...
  const filteredTransactions = useMemo(() => snapshot.transactions.filter((row) => (typeFilter === "all" || row.transaction_type === typeFilter) && matches(row.account_id, row.security_id, row.note)), [snapshot.transactions, typeFilter, matches]);
  ...
  async function saveRecord(resource: string, body: Record<string, unknown>, id?: string) {
    ...
    try {
      ...
      if (resource === "transaction") {
        ...
        for (const key of ["security_mode", "new_security_symbol", "new_security_name", "new_security_market", "new_security_current_price", "new_security_current_price_date"]) delete transactionBody[key];

...
export default function InvestmentsPage() {
  ...
  const accountSummary = selectedAccount ? (() => {
    ...
    const realizedProfit = rows.reduce((sum, row) => sum + row.realized_profit, 0);
    return { account: selectedAccount, stats: [
      { label: "持有成本", value: wholeMoney(costBasis), icon: WalletCards, color: "text-sky-600", bg: "bg-sky-50" },
      { label: "目前市值", value: wholeMoney(marketValue), icon: BriefcaseBusiness, color: "text-indigo-600", bg: "bg-indigo-50" },
      { label: "未實現損益", value: signedWholeMoney(unrealizedProfit), icon: (unrealizedProfit ?? 0) >= 0 ? TrendingUp : TrendingDown, color: (unrealizedProfit ?? 0) >= 0 ? "text-rose-600" : "text-emerald-600", bg: (unrealizedProfit ?? 0) >= 0 ? "bg-rose-50" : "bg-emerald-50" },
      { label: "已實現＋股利", value: signedWholeMoney(realizedProfit), icon: Coins, color: realizedProfit >= 0 ? "text-rose-600" : "text-emerald-600", bg: realizedProfit >= 0 ? "bg-rose-50" : "bg-emerald-50" },
    ] };
  })() : null;
  ...
  return <main className="app-page"><div className="app-page-inner max-w-[1500px]">
    ...
    <section className="app-panel overflow-hidden">
      ...
      {tab !== "settings" && <div className="grid gap-2 border-b border-slate-100 bg-slate-50/70 p-3 sm:grid-cols-2 xl:grid-cols-3"><label className="input input-sm flex items-center gap-2 rounded-lg border-slate-200 bg-white"><Search className="h-4 w-4 text-slate-400" /><input className="grow" placeholder="搜尋股票或代號" value={query} onChange={(event) => setQuery(event.target.value)} /></label><select className="select select-sm rounded-lg border-slate-200 bg-white" value={securityFilter} onChange={(event) => setSecurityFilter(event.target.value)}><option value="all">全部股票</option>{accountTradedSecurities.map((row) => <option key={row.id} value={row.id}>{row.symbol} {row.name}</option>)}</select>{tab === "transactions" && <select className="select select-sm rounded-lg border-slate-200 bg-white" value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as InvestmentTransactionType | "all")}><option value="all">全部類型</option><option value="buy">買進</option><option value="sell">賣出</option></select>}</div>}

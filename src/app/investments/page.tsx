...
export default function InvestmentsPage() {
  ...
  const accountSummaries = snapshot.accounts.map((account) => {
    const rows = snapshot.holdings.filter((row) => row.account_id === account.id);
    const openRows = rows.filter((row) => row.quantity > 0);
    const costBasis = rows.reduce((sum, row) => sum + row.cost_basis, 0);
    const marketValue = openRows.every((row) => row.market_value !== null) ? openRows.reduce((sum, row) => sum + Number(row.market_value), 0) : null;
  ...
  return <main className="app-page"><div className="app-page-inner max-w-[1500px]">
    ...
    <section className="space-y-3">{accountSummaries.length ? accountSummaries.map(({ account, stats }) => <article key={account.id} className="app-panel overflow-hidden"><header className="flex items-center gap-3 border-b border-slate-100 px-4 py-3"><span className="rounded-lg bg-indigo-50 p-2 text-indigo-600"><Landmark className="h-4 w-4" /></span><div className="min-w-0"><h2 className="truncate text-sm font-black text-slate-800">{account.name}</h2><p className="truncate text-xs text-slate-400">{account.broker || "未填券商"}</p></div></header><div className="grid grid-cols-2 gap-px bg-slate-100 xl:grid-cols-4">{stats.map(({ label, value, icon: Icon, color, bg }) => <div key={label} className="flex min-w-0 items-center gap-3 bg-white p-4"><span className={`rounded-lg p-2.5 ${bg} ${color}`}><Icon className="h-5 w-5" /></span><div className="min-w-0"><p className="text-xs font-bold text-slate-500">{label}</p><p className={`truncate text-lg font-black sm:text-xl ${color}`}>{loading ? "—" : value}</p></div></div>)}</div></article>) : <div className="app-panel app-empty">{loading ? "正在讀取帳戶…" : "尚未建立券商帳戶"}</div>}</section>

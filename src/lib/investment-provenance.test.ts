...
describe("phase 5 quote provenance",()=>{
  it("keeps previous-day data explicit without the redundant MIS trade label",()=>{
  ...
  it("shows a concise same-day state before and after market close",()=>{
    expect(quoteDescription({current_price_date:"2026-09-07",current_price_time:"11:00:00",current_price_source:"realtime_trade"},"2026-09-07").freshness).toBe("今日盤中資料");
    expect(quoteDescription({current_price_date:"2026-09-07",current_price_time:"13:30:00",current_price_source:"realtime_trade"},"2026-09-07").freshness).toBe("今日資料");
    expect(quoteDescription({current_price_date:"2026-09-07",current_price_source:"closing"},"2026-09-07").freshness).toBe("今日資料");
  });
  ...
  it("chooses a newer official close over an older MIS trade",async()=>{
    vi.stubGlobal("fetch",vi.fn(async(url:string)=>Response.json(url.includes("getStockInfo")?{msgArray:[{c:"8938",d:"20260904",t:"13:30:00",z:"52.7",ex:"otc"}]}:[{Date:"1150907",SecuritiesCompanyCode:"8938",Close:"53.0"}])));
  ...
  it("retains a valid realtime result if the closing service fails",async()=>{
    vi.stubGlobal("fetch",vi.fn(async(url:string)=>{if(!url.includes("getStockInfo"))throw Error("offline");return Response.json({msgArray:[{c:"8938",d:"20260907",t:"11:00:00",z:"53.2",ex:"otc"}]});}));

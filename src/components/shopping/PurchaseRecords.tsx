...
export function PurchaseRecords({workspaceId,visible,source,onSourceClose,onChanged,onShowWishlist}:{
  workspaceId:string;visible:boolean;source:ShoppingItem|null;onSourceClose:()=>void;onChanged:()=>void;onShowWishlist:()=>void;
}) {
  ...
  async function load() {
    ...
    try { const r=await fetch("/api/purchases?workspace_id="+encodeURIComponent(workspaceId),{cache:"no-store"});const j=await r.json();if(!r.ok)throw Error(j.error);setRows(j.data); }
    catch(e){setError(e instanceof Error?e.message:"讀取失敗");}finally{setLoading(false);}
  }
  useEffect(()=>{if(workspaceId)void load();else setLoading(false);},[workspaceId]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(()=>{
    if(!source){opened.current=null;return;}
    if(loading||error||opened.current===source.id)return;
    opened.current=source.id;
    setDraft(makeDraft(rows.find(r=>r.shopping_item_id===source.id),source));setFormError("");
  },[source,rows,loading,error]);
  function close(){setDraft(null);setFormError("");onSourceClose();}
  async function request(method:string,body:Record<string,unknown>) {
    const r=await fetch("/api/purchases",{method,headers:{"Content-Type":"application/json"},body:JSON.stringify({...body,workspace_id:workspaceId})});
    const j=await r.json();if(!r.ok)throw Error(j.error||"儲存失敗");return j.data;
  }
  async function save() {
    if(!draft||lock.current)return;lock.current=true;setBusy(true);setFormError("");
    try {
      const payload=normalizePurchase(draft,draft.legacy);
      const saved:Purchase=await request(draft.id?"PATCH":"POST",{...payload,id:draft.id,shopping_item_id:draft.shopping_item_id,request_key:draft.request_key});
      setRows(prev=>[saved,...prev.filter(r=>r.id!==saved.id)]);close();onChanged();
      toast({title:"購買紀錄已儲存",description:"不會自動建立記帳。"});
    }catch(e){setFormError(e instanceof Error?e.message:"儲存失敗，請重試");}
    finally{lock.current=false;setBusy(false);}
  }
  async function remove() {
    if(!deleting||lock.current)return;lock.current=true;setBusy(true);setFormError("");
    try {await request("DELETE",{id:deleting.id,restore});setRows(prev=>prev.filter(r=>r.id!==deleting.id));setDeleting(null);onChanged();toast({title:restore?"已刪除紀錄並恢復待購":"購買紀錄已刪除"});}
    catch(e){setFormError(e instanceof Error?e.message:"刪除失敗");}finally{lock.current=false;setBusy(false);}
  }
  async function rebuy(row:Purchase) {
    if(lock.current)return;lock.current=true;setBusy(true);
    const key=rebuyKeys.current.get(row.id) ?? crypto.randomUUID();rebuyKeys.current.set(row.id,key);
    try {await request("POST",{action:"rebuy",id:row.id,request_key:key});rebuyKeys.current.delete(row.id);onChanged();onShowWishlist();toast({title:"已再次加入待購",description:"原購買紀錄仍保留。"});}
    catch(e){toast({variant:"destructive",title:"加入待購失敗",description:e instanceof Error?e.message:"請重試"});}finally{lock.current=false;setBusy(false);}
  }
  const filtered=filterPurchases(rows,query,from,to,store);
  ...
  return <>
    ...
    <section hidden={!visible} className="space-y-4">
      ...
      <div className="grid gap-3 rounded-2xl border bg-white p-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <label className="text-sm">購買迄日<input type="date" className={inputClass} value={to} onChange={e=>setTo(e.target.value)}/></label>
        <label className="text-sm">店家／平台<select className="select select-bordered w-full rounded-xl" value={store} onChange={e=>setStore(e.target.value)}><option value="">全部店家</option>{[...new Set(rows.map(r=>r.store).filter((v):v is string=>!!v))].sort().map(s=><option key={s}>{s}</option>)}</select></label>
        {from&&to&&from>to&&<p role="alert" className="text-red-600">起日不可晚於迄日</p>}
      </div>
      ...
      {loading ? <p role="status">讀取購買紀錄…</p> : !error&&filtered.length===0 ? <div className="rounded-2xl border border-dashed p-8 text-center text-slate-500">沒有符合的購買紀錄。可直接新增，或從待購項目完成購買。</div> :
        <div className="grid gap-3 lg:grid-cols-2">{filtered.map(row=><article key={row.id} className="min-w-0 rounded-2xl border bg-white p-4">
    ...
    {draft&&<Modal title={draft.id?"修改購買紀錄":"記錄這次購買"} onClose={close} busy={busy}>
      <form onSubmit={e=>{e.preventDefault();void save();}} className="space-y-4">
        {source?.sources.length ? <label className="block text-sm">帶入購買來源（不會帶入預估價格）<select className="select select-bordered w-full rounded-xl" defaultValue="" onChange={e=>{const s=source.sources.find(x=>x.id===e.target.value);if(s)setDraft({...draft,store:s.platform||"",url:s.url||""});}}><option value="">手動填寫／選擇來源</option>{source.sources.map(s=><option key={s.id} value={s.id}>{s.platform||s.url||"未命名來源"}</option>)}</select></label>:null}
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="sm:col-span-2">品名<input autoFocus required maxLength={200} className={inputClass} value={draft.name} onChange={e=>setDraft({...draft,name:e.target.value})}/></label>
          <label>購買日期{draft.legacy&&"（可待補）"}<input required={!draft.legacy} type="date" className={inputClass} value={draft.purchase_date} onChange={e=>setDraft({...draft,purchase_date:e.target.value})}/></label>
          <label>數量<input required type="number" min="0.001" step="0.001" className={inputClass} value={draft.quantity} onChange={e=>setDraft({...draft,quantity:e.target.value})}/></label>
          <label>實付總額（選填）<input type="number" min="0" step="0.01" placeholder="不知道可留空" className={inputClass} value={draft.total_amount} onChange={e=>setDraft({...draft,total_amount:e.target.value})}/></label>
          <label>店家／平台<input maxLength={120} className={inputClass} value={draft.store} onChange={e=>setDraft({...draft,store:e.target.value})}/></label>
          <label className="sm:col-span-2">商品連結<input type="url" maxLength={2048} className={inputClass} value={draft.url} onChange={e=>setDraft({...draft,url:e.target.value})}/></label>
          <label className="sm:col-span-2">規格／型號<input maxLength={300} className={inputClass} value={draft.specification} onChange={e=>setDraft({...draft,specification:e.target.value})}/></label>

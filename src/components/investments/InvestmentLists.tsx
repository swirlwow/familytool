...
export function DividendList({ rows, accountMap, securityMap, onEdit, onDelete }: { rows: InvestmentDividend[]; accountMap: Map<string, InvestmentAccount>; securityMap: Map<string, InvestmentSecurity>; onEdit: (row: InvestmentDividend) => void; onDelete: (row: InvestmentDividend) => void }) {
  ...
  return <div className="divide-y divide-slate-100">{rows.map((row) => {
    ...
    return <article key={row.id} className="p-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-center">
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm sm:grid-cols-4 lg:min-w-[560px]"><span><small className="block text-slate-400">股數</small>{numberText(row.eligible_quantity)}</span><span><small className="block text-slate-400">每股股利</small>{priceMoney(row.dividend_per_share)}</span><span><small className="block text-slate-400">預計股利</small><b>{wholeMoney(row.expected_amount)}</b></span><span><small className="block text-slate-400">實收／扣款</small><b>{row.received_amount === null ? "尚未收款" : `${wholeMoney(row.received_amount)}／${wholeMoney(row.deduction_amount)}`}</b></span></div>

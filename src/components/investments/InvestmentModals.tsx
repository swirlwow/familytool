...
export function DividendModal({ modal, accounts, securities, saving, onClose, onSave }: { modal: Extract<InvestmentModal, { kind: "dividend" }>; accounts: InvestmentAccount[]; securities: InvestmentSecurity[]; saving: boolean; onClose: () => void; onSave: Save }) {
  const row = modal.row;
  const [form, setForm] = useState<Record<string, string>>({ account_id: row?.account_id ?? accounts.find((item) => item.is_active)?.id ?? "", security_id: row?.security_id ?? securities.find((item) => item.is_active)?.id ?? "", ex_dividend_date: row?.ex_dividend_date ?? today(), eligible_quantity: row ? String(row.eligible_quantity) : "", dividend_per_share: row ? String(row.dividend_per_share) : "", status: row?.status ?? "pending", payment_date: row?.payment_date ?? "", received_amount: row?.received_amount == null ? "" : String(row.received_amount), deduction_type: row?.deduction_type ?? "unclassified", note: row?.note ?? "" });
  const field = (key: string, value: string) => setForm((current) => ({ ...current, [key]: value }));
  ...
  return <ModalFrame title={row ? "修改股利" : "新增股利"} subtitle="不知道扣款組成時，只填實收金額即可" saving={saving} onClose={onClose} onSubmit={() => void onSave("dividend", form, row?.id)}>
    ...
    <Label title="除權息日期"><input type="date" className={inputClass} value={form.ex_dividend_date} onChange={(event) => field("ex_dividend_date", event.target.value)} /></Label><Label title="計算股數"><input type="number" min="0" step="0.000001" className={inputClass} value={form.eligible_quantity} onChange={(event) => field("eligible_quantity", event.target.value)} /></Label>
    <Label title="每股現金股利"><input type="number" min="0" step="0.000001" className={inputClass} value={form.dividend_per_share} onChange={(event) => field("dividend_per_share", event.target.value)} /></Label><div className="rounded-lg bg-emerald-50 p-3"><span className="text-xs font-bold text-emerald-700">預計股利</span><strong className="mt-1 block text-lg text-emerald-800">{expected.toLocaleString("zh-TW", { style: "currency", currency: "TWD", maximumFractionDigits: 2 })}</strong></div>
    <Label title="狀態"><select className={selectClass} value={form.status} onChange={(event) => field("status", event.target.value)}><option value="pending">待發放</option><option value="received">已收款</option></select></Label><Label title="實際收款日期"><input type="date" className={inputClass} value={form.payment_date} onChange={(event) => field("payment_date", event.target.value)} /></Label>
    <Label title="實際收款金額"><input type="number" min="0" step="0.01" className={inputClass} value={form.received_amount} onChange={(event) => field("received_amount", event.target.value)} /></Label><Label title={`扣除金額（自動：${deduction.toLocaleString("zh-TW", { style: "currency", currency: "TWD" })}）`}><select className={selectClass} value={form.deduction_type} onChange={(event) => field("deduction_type", event.target.value)} disabled={deduction <= 0}><option value="unclassified">尚未確認</option><option value="transfer_fee">匯費</option><option value="nhi">補充保費</option><option value="withholding_tax">扣繳稅</option><option value="other">其他</option></select></Label>
    <Label title="備註" wide><textarea className="textarea textarea-bordered min-h-20 w-full rounded-lg" value={form.note} onChange={(event) => field("note", event.target.value)} /></Label>
  </ModalFrame>;
}

...
import { estimateTradingCosts, resolveDividendStatus } from "@/lib/investments";
import type {
  InvestmentAccount,
...
export function TransactionModal({ modal, accounts, securities, holdings, saving, onClose, onSave }: { modal: Extract<InvestmentModal, { kind: "transaction" }>; accounts: InvestmentAccount[]; securities: InvestmentSecurity[]; holdings: InvestmentHolding[]; saving: boolean; onClose: () => void; onSave: Save }) {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    ...
    account_id: initialAccountId, security_mode: tx || initialType === "sell" || securities.length ? "existing" : "new",
    security_id: tx?.security_id ?? (initialType === "sell" ? initialHoldingSecurityId : undefined) ?? securities.find((row) => row.is_active)?.id ?? "", new_security_symbol: "", new_security_name: "",
  ...
  const useNewSecurity = type === "buy" && form.security_mode === "new";
  const isSellOption = (securityId: string, accountId: string) =>
    holdings.some((holding) => holding.account_id === accountId && holding.security_id === securityId && holding.quantity > 0)
    || (tx?.transaction_type === "sell" && tx.account_id === accountId && tx.security_id === securityId);
  const selectableSecurities = type !== "sell" ? securities : securities.filter((security) => isSellOption(security.id, form.account_id));
  const updateType = (value: string) => setForm((current) => {
    if (value !== "sell") return { ...current, transaction_type: value };
    const choices = securities.filter((security) => isSellOption(security.id, current.account_id));
    return { ...current, transaction_type: value, security_mode: "existing", security_id: choices.some((security) => security.id === current.security_id) ? current.security_id : choices[0]?.id ?? "" };
  });
  const updateAccount = (value: string) => setForm((current) => {
    if (current.transaction_type !== "sell") return { ...current, account_id: value };
    const choices = securities.filter((security) => isSellOption(security.id, value));
    return { ...current, account_id: value, security_id: choices.some((security) => security.id === current.security_id) ? current.security_id : choices[0]?.id ?? "" };
  });
  ...
  return <ModalFrame title={tx ? "修改交易" : type === "sell" ? "新增賣出" : "新增買進"} subtitle={type === "sell" ? "只顯示所選券商帳戶目前持有的股票" : useNewSecurity ? "儲存時會同時建立股票資料" : "可修改後重新計算後續持股"} saving={saving} onClose={onClose} onSubmit={() => void onSave("transaction", { ...form, fee: effectiveFee, tax: effectiveTax }, tx?.id)}>
    <Label title="交易類型"><select className={selectClass} value={type} onChange={(event) => updateType(event.target.value)}><option value="buy">買進</option><option value="sell">賣出</option></select></Label>
    ...
    <Label title="券商帳戶" wide><AccountSelect value={form.account_id} onChange={updateAccount} accounts={accounts} includeId={tx?.account_id} /></Label>
    <fieldset className="grid gap-3 rounded-xl border border-indigo-100 bg-indigo-50/35 p-3 sm:col-span-2 sm:grid-cols-2"><div className="flex items-center justify-between gap-3 sm:col-span-2"><span className="text-sm font-black text-slate-800">股票資料</span>{!tx && type === "buy" && securities.length > 0 && <div className="inline-flex rounded-lg bg-white p-1 shadow-sm"><button type="button" aria-pressed={!useNewSecurity} className={`rounded-md px-3 py-1.5 text-xs font-bold ${!useNewSecurity ? "bg-indigo-600 text-white" : "text-slate-500"}`} onClick={() => field("security_mode", "existing")}>選擇既有</button><button type="button" aria-pressed={useNewSecurity} className={`rounded-md px-3 py-1.5 text-xs font-bold ${useNewSecurity ? "bg-indigo-600 text-white" : "text-slate-500"}`} onClick={() => field("security_mode", "new")}>建立新股票</button></div>}</div>
      {useNewSecurity ? <><Label title="股票代號"><input className={`${inputClass} bg-white uppercase`} value={form.new_security_symbol} onChange={(event) => field("new_security_symbol", event.target.value)} placeholder="例如：2330" /></Label><Label title="股票名稱"><input className={`${inputClass} bg-white`} value={form.new_security_name} onChange={(event) => field("new_security_name", event.target.value)} /></Label><Label title="市場"><select className={`${selectClass} bg-white`} value={form.new_security_market} onChange={(event) => field("new_security_market", event.target.value)}><option value="TWSE">TWSE（台股上市）</option><option value="TPEx">TPEx（台股上櫃）</option><option value="US">US（美股）</option><option value="OTHER">OTHER（其他）</option></select></Label><Label title="目前股價（選填）"><input type="number" min="0" step="0.01" className={`${inputClass} bg-white`} value={form.new_security_current_price} onChange={(event) => field("new_security_current_price", event.target.value)} /></Label></> : <Label title="選擇股票" wide><SecuritySelect value={form.security_id} onChange={(value) => field("security_id", value)} securities={selectableSecurities} includeId={type === "sell" && tx?.transaction_type === "sell" ? tx.security_id : undefined} /></Label>}
      {type === "sell" && selectableSecurities.length === 0 && <p className="text-xs font-medium text-amber-700 sm:col-span-2">此券商帳戶目前沒有可賣出的持股。</p>}
    </fieldset>
    {type === "dividend" ? <Label title="股利金額" wide><input type="number" min="0" step="0.01" className={inputClass} value={form.cash_amount} onChange={(event) => field("cash_amount", event.target.value)} /></Label> : <><Label title="股數"><input type="number" min="0" step="0.000001" className={inputClass} value={form.quantity} onChange={(event) => field("quantity", event.target.value)} /></Label><Label title="成交價"><input type="number" min="0" step="0.01" className={inputClass} value={form.price} onChange={(event) => field("price", event.target.value)} /></Label></>}
    {type !== "dividend" && <><Label title="手續費（自動試算）"><input type="number" min="0" step="1" className={inputClass} value={effectiveFee} onChange={(event) => editCost("fee", event.target.value)} /></Label><Label title="交易稅（自動試算）"><input type="number" min="0" step="1" className={inputClass} value={effectiveTax} onChange={(event) => editCost("tax", event.target.value)} /></Label><div className="flex items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-500 sm:col-span-2"><span>{selectedSecurity?.market === "TWSE" || selectedSecurity?.market === "TPEx" ? "手續費以 0.1425% 參考費率試算；實際費率與最低收費依券商為準，可手動修改。" : "非台股市場不自動估算費稅，請依券商資料填寫。"}</span><button type="button" className="btn btn-ghost btn-xs shrink-0 text-indigo-700" onClick={resetAutomaticCosts}>重新自動計算</button></div><Label title={type === "buy" ? "實際扣款（選填）" : "實際入帳（選填）"}><input type="number" min="0" step="0.01" className={inputClass} value={form.settlement_amount} onChange={(event) => field("settlement_amount", event.target.value)} placeholder={estimatedSettlement > 0 ? `試算 ${estimatedSettlement.toFixed(0)}` : ""} /></Label><Label title="委託單號（同帳戶、同交易日不可重複）"><input className={inputClass} value={form.order_number} onChange={(event) => field("order_number", event.target.value)} /></Label></>}
...
export function DividendModal({ modal, accounts, securities, saving, onClose, onSave }: { modal: Extract<InvestmentModal, { kind: "dividend" }>; accounts: InvestmentAccount[]; securities: InvestmentSecurity[]; saving: boolean; onClose: () => void; onSave: Save }) {
  const row = modal.row;
  const initialPaymentDate = row?.payment_date ?? "";
  const initialDividendType = row?.dividend_type ?? "cash";
  const initialStatus = resolveDividendStatus(initialDividendType, initialPaymentDate || null, row?.status ?? "pending", today());
  const [form, setForm] = useState<Record<string, string>>({ account_id: row?.account_id ?? modal.accountId ?? accounts.find((item) => item.is_active)?.id ?? "", security_id: row?.security_id ?? securities.find((item) => item.is_active)?.id ?? "", dividend_type: initialDividendType, ex_dividend_date: row?.ex_dividend_date ?? today(), eligible_quantity: row ? String(row.eligible_quantity) : "", dividend_per_share: row ? String(row.dividend_per_share) : "", stock_dividend_rate: row ? String(row.stock_dividend_rate) : "", status: initialStatus, payment_date: initialPaymentDate, received_amount: row?.received_amount == null ? "" : String(row.received_amount), shares_received: row?.shares_received == null ? "" : String(row.shares_received), deduction_type: row?.deduction_type ?? "unclassified", note: row?.note ?? "" });
  const field = (key: string, value: string) => setForm((current) => ({ ...current, [key]: value }));
  const updatePaymentDate = (value: string) => setForm((current) => ({
    ...current,
    payment_date: value,
    status: resolveDividendStatus(current.dividend_type === "stock" ? "stock" : "cash", value || null, current.status === "received" ? "received" : "pending", today()),
  }));
